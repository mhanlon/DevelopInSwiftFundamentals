 ...
 */
// Int(String) returned nil when int encountered a non-digit while parsing the string
 ...
 */
//print prints the optional (which in turn prints its value), not directly the wrapped Int.
print(userAge!)
/*:
 Now use optional binding to unwrap `userAge`. If `userAge` has a value, print it to the console.
 */
if let uUserAge = userAge{
    print(uUserAge)
}
