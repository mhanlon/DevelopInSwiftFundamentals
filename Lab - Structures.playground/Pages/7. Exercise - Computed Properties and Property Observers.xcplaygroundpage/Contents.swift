...
let test = Rectangle(width: 23, height: 4)
print(test.area)
/*:
...
struct Height {
    var heightInInches: Double {
        didSet{
            let newCm = self.heightInInches * 2.54
            if self.heightInCentimeters != newCm {
                self.heightInCentimeters = newCm
            }
        }
    }
    ...
    var heightInCentimeters: Double {
        didSet{
            let newInches = self.heightInCentimeters / 2.54
            if self.heightInInches != newInches {
                self.heightInInches = newInches
            }
        }
    }
...
var hght = Height(heightInCentimeters: 200)
print(hght.heightInInches)
hght.heightInCentimeters = 300
print(hght.heightInInches)
