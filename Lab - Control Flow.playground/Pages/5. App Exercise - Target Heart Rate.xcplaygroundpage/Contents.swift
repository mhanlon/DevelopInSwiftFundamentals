...
let isInTarget = "You're right on track!"
let isBelowTarget = "You're doing great, but try to push it a bit!"
let isAboveTarget = "You're on fire! Slow it down just a bit."

if currentHR < 120 {
    print(isBelowTarget)
} else if currentHR >= 120 && currentHR <= 150 {
    print(isInTarget)
} else {
    print(isAboveTarget)
}
