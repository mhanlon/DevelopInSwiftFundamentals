...
print(calculateArea(x: 2.0, y: 4.0))
print(calculateArea(x: -2.0, y: 4.0))
/*:
...
func createUser() -> User? {
    guard let ufn = firstNameTextField.text,
          let uln = lastNameTextField.text,
          let uage = ageTextField.text else{
        return nil
    }
    return User(firstName: ufn, lastName: uln, age: uage)
}
/*:
...
let rv = createUser()
if let urv = rv {
    print("firstName: \(urv.firstName) lastName: \(urv.lastName) age: \(urv.age)")
}
